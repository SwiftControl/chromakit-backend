...
class HistogramResponse(BaseModel):
    ...
    histogram: dict[str, list[int]] = Field(
        ...
        example={"red": [0, 5, 10, 15, 20], "green": [0, 3, 8, 12, 18], "blue": [0, 2, 6, 14, 22]},
    )
...
class ProcessingOperationResponse(BaseModel):
    ...
    id: str = Field(
        ..., description="Unique identifier of the processed image", example="img_processed_123456"
    )
    url: str = Field(
        ...,
        description="Public URL to access the processed image",
        example="https://api.example.com/images/img_123456/download",
    )
    width: int | None = Field(
        None, description="Width of the processed image in pixels", example=800
    )
    height: int | None = Field(
        None, description="Height of the processed image in pixels", example=600
    )
    mime_type: str | None = Field(
        None, description="MIME type of the processed image", example="image/png"
    )
    operation: str = Field(
        ..., description="The processing operation that was applied", example="brightness"
    )
    parameters: dict = Field(
        default_factory=dict, description="Parameters used for the processing operation"
    )
    original_image_id: str = Field(
        ..., description="ID of the original image that was processed", example="img_123456"
    )
    created_at: str | None = Field(
        None, description="ISO timestamp when the processed image was created"
    )
