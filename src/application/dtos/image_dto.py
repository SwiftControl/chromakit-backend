...
class ImageMetadata(BaseModel):
    ...
    path: str = Field(
        ..., description="Storage path of the image file", example="user123/img_123456.png"
    )
    ...
    created_at: datetime = Field(
        ..., description="ISO timestamp when the image was created or uploaded"
    )
    original_id: str | None = Field(
        None, description="ID of the original image if this is a processed version"
    )
    original_filename: str | None = Field(
        None, description="Original filename when uploaded", example="photo.jpg"
    )
    file_size: int | None = Field(
        None, description="Size of the image file in bytes", example=2048576
    )
    url: str | None = Field(
        None,
...
class ListImagesResponse(BaseModel):
    ...
    limit: int = Field(
        ...,
        description="Maximum number of images returned in this response",
        example=20,
        ge=1,
        le=100,
    )
    offset: int = Field(
        ..., description="Number of images skipped from the beginning", example=0, ge=0
    )
...
class ProcessImageResponse(BaseModel):
    ...
    operation: str = Field(
        ..., description="Name of the processing operation applied", example="brightness"
    )
...
class BrightnessRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to adjust brightness", example="img_123456"
    )
    factor: float = Field(
        ...,
...
class ContrastRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to adjust contrast", example="img_123456"
    )
    type: str = Field(
        ...,
        description="Type of contrast adjustment",
        example="logarithmic",
        pattern="^(logarithmic|exponential)$",
    )
...
class ChannelRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to modify channels", example="img_123456"
    )
    channel: str = Field(
        ...,
        description="Color channel to manipulate",
        example="red",
        pattern="^(red|green|blue|cyan|magenta|yellow)$",
    )
    enabled: bool = Field(
...
class GrayscaleRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to convert to grayscale", example="img_123456"
    )
    method: str = Field(
        ...,
        description="Grayscale conversion method",
        example="luminosity",
        pattern="^(average|luminosity|midgray)$",
    )
...
class BinarizeRequest(BaseModel):
    ...
    threshold: float = Field(
        ..., description="Binarization threshold (0.0 to 1.0)", example=0.5, ge=0.0, le=1.0
    )
...
class RotateRequest(BaseModel):
    ...
    angle: float = Field(
        ..., description="Rotation angle in degrees (positive = counterclockwise)", example=45.0
    )
...
class ReduceResolutionRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to reduce resolution", example="img_123456"
    )
    factor: int = Field(
        ...,
        description="Reduction factor (2 = half size, 3 = one third size, etc.)",
        example=2,
        ge=2,
        le=10,
    )
...
class EnlargeRegionRequest(BaseModel):
    ...
    image_id: str = Field(
        ..., description="ID of the image to enlarge a region from", example="img_123456"
    )
    ...
    zoom_factor: int = Field(
        ..., description="Enlargement factor for the selected region", example=2, ge=1, le=10
    )
...
class MergeImagesRequest(BaseModel):
    ...
    image1_id: str = Field(
        ..., description="ID of the first image (base image)", example="img_123456"
    )
    image2_id: str = Field(
        ..., description="ID of the second image (overlay image)", example="img_789012"
    )
    transparency: float = Field(
        ...,
        description=(
            "Transparency level for the overlay " "(0.0 = fully transparent, 1.0 = fully opaque)"
        ),
        example=0.5,
        ge=0.0,
        le=1.0,
